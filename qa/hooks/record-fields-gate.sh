...
# record file docs/issue-<n>/reports/qa.md.
#
...
if not re.match(r'^docs/issue-[0-9]+/reports/qa\.md$', rel):
    allow()
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "qa-cycle: refused — fail-closed: internal error (record-fields-gate.sh judge exited $rc)." >&2
  exit 2
fi
exit "$rc"
