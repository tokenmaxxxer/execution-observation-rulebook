...
# Kill switch: export QA_CYCLE_OFF=1
set -uo pipefail

deny() { echo "qa: refused — $1" >&2; exit 2; }

case "${QA_CYCLE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
QA_PAYLOAD="$payload" \
QA_CPD="${CLAUDE_PROJECT_DIR:-}" \
QA_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("qa: refused — %s\n" % msg)
    sys.exit(2)
...
def _qa_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("qa: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _qa_fail_closed
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "qa: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
