---
description: Trust accounting — follow every qa PR from the run records to its outcome (merged / commented / closed unmerged) and every user-filed issue that followed; report acceptance rate, noise rate, and backlog
...
(`docs/issue-<n>/reports/qa/runs/`) and queries the qa PRs' outcomes (and any issues the USER filed from them — qa itself files none under contract v3).

## 1. Collect

Read every `docs/issue-<n>/reports/qa/runs/*.md` for the current
subject (with `--all`, every subject under `docs/issue-<n>/reports/` that has
...
`gh pr view <n> --json state,mergedAt,closedAt` (and `gh issue view` only for issues the user filed). Classify:
