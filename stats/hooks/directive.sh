...
Rules:
- READ-ONLY. `/qa-stats` never writes a run record, an evidence file, an issue, or the cycle state file (`state.md`). It only reads `docs/reports/records/<subject>/qa/runs/*.md` and queries the tracker via `gh`.
- NO TRANSITION OWNERSHIP. `stats` owns no transition in the QA cycle state machine and never writes `docs/reports/records/<subject>/qa/state.md` or any `.verdict-token` file, under any circumstance, even when its own report would seem to justify one (e.g. a high acceptance rate does not authorize a `handed-off` or `verified-fixed` write — that belongs to `qa-cycle`/`signoff`).
