...
Usage: install.sh
  ...
  Installs the tokenmaxxxer QA stack for your account only. Applies to every
...
# Resolve the target path and confirm it stays inside the user's home
# directory before anything is written.
home = os.path.realpath(os.path.expanduser("~"))
path = os.path.expanduser(path)
resolved = os.path.realpath(path)
if resolved != home and not resolved.startswith(home + os.sep):
    sys.exit(f"ERROR: refusing to write outside the home directory: {resolved}")
...
if os.path.exists(path):
    with open(path) as f:
        try:
            ...
            # Parse failure: abort leaving the original file untouched rather
            # than overwriting it with a fresh/partial settings object.
