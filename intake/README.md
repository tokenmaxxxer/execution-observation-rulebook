...
Per-subject QA profile. `/qa-init` reads the target project once — where bugs
...
into a single file committed in the target repo itself:
`docs/reports/records/<subject>/qa/intake.md`. Every
other plugin in the stack reads that file instead of rediscovering (or
guessing) per session.

This is what makes the stack work *per subject, for every session*: run
`/qa-init` once and the profile is committed to the target repo; from then
...
- **`/qa-init`** — discovery: git remote → issue repo, `.github/ISSUE_TEMPLATE/*`,
  ...
  tracker repo, staging URL, language). Writes the profile in-repo; re-running
  updates stale fields and preserves human edits.
- **`/qa-init --check`** — doctor: profile present, `gh`
