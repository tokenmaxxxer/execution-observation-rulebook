---
description: Launch the product and execute QA — plan-driven if qa/plan.md exists, ad-hoc smoke otherwise
...
Read `qa/intake.md`. If it is missing, do a one-pass ad-hoc discovery (launch
command, base URL) and say in one line that you are running without a profile.
...
- `qa/plan.md` exists → execute its cases, filtered by the scope argument.
...
- evidence: command + output, screenshot path, or log excerpt — stored under
  `qa/evidence/<run-slug>/`

Verdicts follow the testrun directive: nothing you didn't run gets a verdict.

## 5. Record

Write `qa/runs/<YYYY-MM-DD>-<slug>.md`:

```markdown
# Run <date> — <scope>

profile: qa/intake.md (or "none")
