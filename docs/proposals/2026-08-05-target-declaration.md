...
- Judging the declaration's structure from a truncated read did not hold as safe: capping the read at 64KB and then counting `---` blocks over that prefix let an oversized `target.md` with a second, genuinely ambiguous block flip the gate's verdict from refuse to allow purely because the second block fell past the cap — a truncated read is never a verdict, and the fix (refuse whenever the file exceeds what the gate actually reads) is recorded in `docs/reports/2026-08-05-target-declaration-run.md`.
