...
- First pass added `--dump-facts` as an unconditional exit-0 branch keyed only on `$1`, which let a payload carrying that flag as its first argument skip adjudication entirely — a before-landing hunt reproduced this as a token-gated `handed-off -> re-verifying` write allowed with no token present.
