...
스택이 만드는 모든 것은 **대상 레포 자체 안**, qa 소유 레코드 영역에
남습니다 (`docs/specs/role-handoff-contract.md` §10 기준) — 작업 하나당
subject 디렉토리 하나. 세션이 죽어도 디스크에서 재개되고, 다른 어떤
role(또는 새 QA 세션)이든 외부 호스트 경로 없이 대상 레포 클론 하나만으로
qa의 전체 레코드를 읽을 수 있습니다:

```
docs/reports/records/<subject>/
  qa.md                  # 블랙보드 레코드: verdict, 포인터, 공통 헤더
  qa/
    intake.md             # 프로파일 (env var는 이름만 — 시크릿 절대 금지)
    plan.md               # 선택적 테스트 플랜 (로드맵: /testplan이 작성)
    state.md               # 아이템별 상태 머신 (docs/handbooks/qa-cycle.md)
    tokens/                 # verdict 토큰 — signoff가 사용자 본인 턴에서 발행
    runs/                 # 실행당 기록 하나: 케이스 표, 실패, 이슈 URL
    evidence/             # run record가 인용하는 스크린샷·출력·로그
    regress/              # 채택된 회귀 테스트 — /testrun이 매 실행마다 돌림
```

설계상 프로젝트 쪽에 남는 두 가지는 그대로입니다: **버그 리포트**는 각
프로젝트 자체 트래커로 갑니다(트래커가 QA→개발 인수인계 창구이므로
개발자가 보는 곳에 있어야 합니다). 채택된 회귀 테스트는 프로젝트가 원하면
업스트림 PR로 그쪽 CI에 *추가로* 넣을 수 있습니다. 세션 도중 필요한 일회성
작업 파일(커밋할 의도가 없는 스크래치)은 세션 임시 디렉토리(`mktemp -d`)를
쓰고, 세션보다 오래 남는 별도 레포나 호스트-로컬 경로는 절대 쓰지 않습니다.

## 설치

스택은 대상 레포(또는 그 레포를 상대로 QA를 돌리는 환경)에 다른 플러그인과
똑같이 설치합니다 — 이제 별도로 프로비저닝하거나 env var로 가리켜야 할
외부 워크스페이스 레포가 없습니다.
